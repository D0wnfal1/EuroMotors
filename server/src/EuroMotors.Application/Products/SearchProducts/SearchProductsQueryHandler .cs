...
internal sealed class SearchProductsByCarModelIdQueryHandler(IDbConnectionFactory dbConnectionFactory)
    : IQueryHandler<SearchProductsQuery, Pagination<ProductResponse>>
{
    public async Task<Result<Pagination<ProductResponse>>> Handle(SearchProductsQuery request, CancellationToken cancellationToken)
    {
